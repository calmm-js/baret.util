...
import K, * as U from "../dist/baret.util.cjs"
...
const testEq = (exprIn, expect) => {
  ...
  return it(`${expr} => ${show(expect)}`, done => {
    const actual =
      eval(`(K, Bacon, R, U, C) => ${expr}`)(K, Bacon, R, U, Bacon.constant)
    const check = actual => {
...
describe("view", () => {
  testEq(`U.view("x", Bacon.constant({x: 101}))`, 101)
  testEq(`U.view(Bacon.constant("x"), Bacon.constant({x: 101}))`, 101)
  testEq(`U.view("x", {x: 101})`, 101)
  testEq(`U.view("x", U.atom({x: 101}))`, 101)
  testEq(`U.view(Bacon.constant("x"), U.atom({x: 101}))`, 101)
})
...
describe("mapCached", () => {
  testEq(`U.seq(C([2, 1, 1]).toEventStream().concat(Bacon.later(1, [1, 3, 2])),
                U.toProperty,
...
describe("Context", () => {
  ...
  testRender(<U.Context context={{who: Bacon.constant("World")}}>
               <Who/>
             </U.Context>,
             '<div>Hello, World!</div>')
})
...
describe("bus", () => {
  testEq(`{const b = U.bus();
           let result;
           b.flatMap(R.negate).onValue(x => result = x);
           b.push(-101);
           return result}`,
         101)
  testEq(`{const b = U.bus();
           let result;
           b.flatMapError(R.negate).onValue(x => result = x);
           b.error(-69);
           return result}`,
         69)
  testEq(`{const b = U.bus();
